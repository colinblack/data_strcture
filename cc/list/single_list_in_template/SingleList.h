#define SingleList __declspec(dllexport)
#include "ListNode.h"
...
template<typename Type> void SingleList<Type>::Merge(SingleList<Type>* a, SingleList<Type>* b)
{
	ListNode<Type>* cur = this->head;
	if (cur->m_pnext != NULL)
	{
		cout << "this list is no empty" << endl;
		return;
	}
	ListNode<Type>* na = a->head->m_pnext;
	ListNode<Type>* nb = b->head->m_pnext;

	while (na != NULL && nb != NULL)
	{
		if (na->m_data < nb->m_data)
		{
			cur->m_pnext = na;
			na = na->m_pnext;
		}
		else if (na->m_data >= nb->m_data)
		{
			cur->m_pnext = nb;
			nb = nb->m_pnext;
		}
